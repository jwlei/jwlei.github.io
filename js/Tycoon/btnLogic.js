    ...
    function () {
        ...
        function KobleSpill(funksjonEtterKobling = function () { }) {
            rememberGame.databaseRef.on('value', function (snapshot) {
                ...
                if (ny != null) {
                    ...
                    rememberGame.spillere = SpillerBoms.Spillere_set(ny.spillere);
        ...
        function AutoSpiller() {
            if (rememberGame.status == "G") {
                rememberGame.spillere.filter(s => (s.spillerKey == "PC")).forEach(spiller => {
                    AutoSpiller2(rememberGame, spiller);
                });
            } else if (rememberGame.status == "S" && rememberGame.sinTur >= 0 && rememberGame.spillere[rememberGame.sinTur].spillerKey == "PC") {
                AutoSpiller2(rememberGame, rememberGame.spillere[rememberGame.sinTur]);
            }
            if (rememberGame.status != "F") { setTimeout(AutoSpiller, 1500); }
        }
        ...
        document.getElementById('dialogSporOmNickOk').addEventListener('click', function () {
            ...
            if (onskeNick != "") {
                ...
                var nyspiller = rememberGame.NySpiller(new SpillerBoms(onskeNick, key));
        ...
        document.getElementById('BnFNyAutoSpiller').addEventListener('click', function () {
            ...
            if (alternativer.length > 0) {
                rememberGame.NySpiller(new SpillerBoms(alternativer[0], key));
            } else {
